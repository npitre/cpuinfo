 ...
 *
 * cpuinfo initialization for bare-metal AArch64 targets, i.e. an RTOS or no
 * OS at all, where the code runs at EL1.
 *
 * ISA features are read straight out of the AArch64 ID registers rather than
 * from /proc/cpuinfo or HWCAP.  That is not merely an alternative to the Linux
 * path, it is the only option here: there is no kernel publishing HWCAP, and
 * conversely the ID registers are inaccessible from Linux userspace.  Reading
 * them requires EL1, which is exactly where an RTOS application runs.
 *
 * Nothing below is specific to one RTOS.  The only external input is the core
 * count, obtained through get_num_cpus() below; add a case there to support
 * another RTOS.
 *
 * Topology is simplified to a single package with a single cluster.
 */
...
static inline uint32_t get_num_cpus(void) {
	return (uint32_t)arch_num_cpus();
}

#else

/*
 * No RTOS-specific way to ask: assume a single core.  SMP-capable ports should
 * add a case above rather than change this default.
 */
#define get_num_cpus() 1u
...
void cpuinfo_arm_baremetal_init(void) {
	...
	const uint32_t core_count = get_num_cpus();
	...
	strncpy(static_package.name, "ARM AArch64", CPUINFO_PACKAGE_NAME_MAX);
